...
def create_app():
	...
	if app.config['LOG_TO_STDOUT']:
		...
		if not os.path.exists('logs'):
			os.mkdir('logs')
		file_handler = RotatingFileHandler('logs/budget.log', maxBytes=10240, backupCount=10)
		file_handler.setFormatter(logging.Formatter('%(asctime)s %(levelname)s: %(message)s ''[in %(pathname)s:%(lineno)d]'))
		file_handler.setLevel(logging.INFO)
		app.logger.addHandler(file_handler)

	app.logger.setLevel(logging.INFO)
	app.logger.info('Budget Tracker')
...
def register_make_shell_context(app):
	...
	def make_shell_context():
		return {'db': models.db, 'Users': models.Users, 'Entry': models.Entry}